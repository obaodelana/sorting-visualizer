...
// Compiler complains of ignoring return value of certain functions (fgets, fread)
size_t satisfyCompiler;
char *satisfyCompilerText;
...
void GuiLoadStyle(const char *fileName)
{
    ...
    {
        char buffer[256] = { 0 };
        satisfyCompilerText = fgets(buffer, 256, rgsFile);
    ...
    if (tryBinary)
    {
        ...
        satisfyCompiler = fread(signature, 1, 4, rgsFile);
        satisfyCompiler = fread(&version, 1, sizeof(short), rgsFile);
        satisfyCompiler = fread(&reserved, 1, sizeof(short), rgsFile);
        satisfyCompiler = fread(&propertiesCount, 1, sizeof(int), rgsFile);
            ...
            (signature[3] == ' '))
        {
            ...
            {
                satisfyCompiler = fread(&controlId, 1, sizeof(short), rgsFile);
                satisfyCompiler = fread(&propertyId, 1, sizeof(short), rgsFile);
                satisfyCompiler = fread(&propertyValue, 1, sizeof(int), rgsFile);
            ...
            int fontDataSize = 0;
            satisfyCompiler = fread(&fontDataSize, 1, sizeof(int), rgsFile);
            ...
            if (fontDataSize > 0)
            {
                ...
                satisfyCompiler = fread(&font.baseSize, 1, sizeof(int), rgsFile);
                satisfyCompiler = fread(&font.charsCount, 1, sizeof(int), rgsFile);
                satisfyCompiler = fread(&fontType, 1, sizeof(int), rgsFile);

                // Load font white rectangle
                satisfyCompiler = fread(&whiteRec, 1, sizeof(Rectangle), rgsFile);

                // Load font image parameters
                int fontImageSize = 0;
                satisfyCompiler = fread(&fontImageSize, 1, sizeof(int), rgsFile);

                if (fontImageSize > 0)
                {
                    Image imFont = { 0 };
                    imFont.mipmaps = 1;
                    satisfyCompiler = fread(&imFont.width, 1, sizeof(int), rgsFile);
                    satisfyCompiler = fread(&imFont.height, 1, sizeof(int), rgsFile);
                    satisfyCompiler = fread(&imFont.format, 1, sizeof(int), rgsFile);

                    imFont.data = (unsigned char *)RAYGUI_MALLOC(fontImageSize);
                    satisfyCompiler = fread(imFont.data, 1, fontImageSize, rgsFile);
                ...
                for (int i = 0; i < font.charsCount; i++) satisfyCompiler = fread(&font.recs[i], 1, sizeof(Rectangle), rgsFile);

                // Load font chars info data
                font.chars = (CharInfo *)RAYGUI_CALLOC(font.charsCount, sizeof(CharInfo));
                for (int i = 0; i < font.charsCount; i++)
                {
                    satisfyCompiler = fread(&font.chars[i].value, 1, sizeof(int), rgsFile);
                    satisfyCompiler = fread(&font.chars[i].offsetX, 1, sizeof(int), rgsFile);
                    satisfyCompiler = fread(&font.chars[i].offsetY, 1, sizeof(int), rgsFile);
                    satisfyCompiler = fread(&font.chars[i].advanceX, 1, sizeof(int), rgsFile);
                }
...
char **GuiLoadIcons(const char *fileName, bool loadIconsName)
{
    ...
    if (rgiFile != NULL)
    {
        ...
        satisfyCompiler = fread(signature, 1, 4, rgiFile);
        satisfyCompiler = fread(&version, 1, sizeof(short), rgiFile);
        satisfyCompiler = fread(&reserved, 1, sizeof(short), rgiFile);
        satisfyCompiler = fread(&iconsCount, 1, sizeof(short), rgiFile);
        satisfyCompiler = fread(&iconsSize, 1, sizeof(short), rgiFile);
            ...
            (signature[3] == ' '))
        {
            if (loadIconsName)
            {
                ...
                for (int i = 0; i < iconsCount; i++)
                {
                    ...
                    satisfyCompiler = fread(guiIconsName[i], 32, 1, rgiFile);
                }
            }

            // Read icons data directly over guiIcons data array
            satisfyCompiler = fread(guiIcons, iconsCount*(iconsSize*iconsSize/32), sizeof(unsigned int), rgiFile);
        }
